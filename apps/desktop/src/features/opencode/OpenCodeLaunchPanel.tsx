    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-6 py-8"><div className="mx-auto max-w-4xl">
      ...
      <Card padding="lg">
        ...
        <div className="mt-4 grid gap-1 rounded-[22px] p-1 sm:grid-cols-3" style={{ background: "var(--rc-chip-inset-bg)", boxShadow: "var(--rc-chip-inset-shadow)" }}>
          <ModeOption mode="bundled" active={draft.mode === "bundled"} title="内置 OpenCode" description="小妍自带的官方 harness，无需安装" onSelect={(mode) => onDraftChange("mode", mode)} />
        ...
        {draft.mode === "bundled" && !runtime.snapshot?.bundledAvailable && <div className="mt-4 flex gap-2.5 rounded-2xl border border-amber-700/15 bg-amber-50/60 px-3.5 py-3 text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0" /><p className="text-xs leading-5">{runtime.snapshot?.pathAvailable ? "当前构建未包含内置 OpenCode 运行时，将回退到已安装版本。可执行 pnpm opencode:prepare-runtime 生成内置运行时。" : "当前构建未包含内置 OpenCode 运行时，也未在环境中发现已安装版本。可执行 pnpm opencode:prepare-runtime 生成内置运行时，或按官方方式安装 OpenCode。"}</p></div>}
        {draft.mode === "bundled" && runtime.snapshot?.bundledExecutable && <p className="mt-3 text-xs text-ink-tertiary">内置运行时 {runtime.snapshot.bundledExecutable}</p>}
