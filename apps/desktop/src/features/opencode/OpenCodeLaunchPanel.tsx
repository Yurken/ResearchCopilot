...
export default function OpenCodeLaunchPanel({ runtime, draft, onDraftChange }: { runtime: ReturnType<typeof useOpenCodeRuntime>; draft: OpenCodeRuntimeConfig; onDraftChange: <K extends keyof OpenCodeRuntimeConfig>(key: K, value: OpenCodeRuntimeConfig[K]) => void }) {
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-6 py-8"><div className="mx-auto max-w-3xl">
      ...
      <Card padding="lg">
        ...
        <div className="mt-4 grid gap-1 rounded-[22px] p-1 sm:grid-cols-2" style={{ background: "var(--rc-chip-inset-bg)", boxShadow: "var(--rc-chip-inset-shadow)" }}>
          <ModeOption mode="path" active={draft.mode === "path"} title="本机 OpenCode" description="使用环境中已安装的版本" onSelect={(mode) => onDraftChange("mode", mode)} />
          <ModeOption mode="external" active={draft.mode === "external"} title="外部 OpenCode" description="指定自行维护的可执行文件" onSelect={(mode) => onDraftChange("mode", mode)} />
        </div>
        {draft.mode === "path" && !runtime.snapshot?.pathAvailable && <div className="mt-4 flex gap-2.5 rounded-2xl border border-amber-700/15 bg-amber-50/60 px-3.5 py-3 text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0" /><p className="text-xs leading-5">未找到 OpenCode。请先按官方方式安装，或选择外部可执行文件。</p></div>}
