...
import { DEFAULT_OPENCODE_CONFIG, formatOpenCodeError, type OpenCodeApiImportResult, type OpenCodeRuntimeConfig, type OpenCodeRuntimeSnapshot } from "./shared";
...
export function useOpenCodeRuntime() {
  ...
  const [apiImportResult, setApiImportResult] = useState<OpenCodeApiImportResult | null>(null);
  ...
  const configureAndImportXiaoyanApi = useCallback(async (config: OpenCodeRuntimeConfig) => {
    const request = ++sequence.current;
    beginOperation();
    setError("");
    setApiImportResult(null);
    try {
      const configured = await invoke<OpenCodeRuntimeSnapshot>("opencode_runtime_configure", { config });
      const result = await invoke<OpenCodeApiImportResult>("opencode_runtime_import_xiaoyan_api");
      ...
      if (request === sequence.current) setError(formatOpenCodeError(cause));
      return null;
    } finally {
      endOperation();
    }
  ...
  return { snapshot, config: snapshot?.config ?? DEFAULT_OPENCODE_CONFIG, loading, busy, error, apiImportResult, refresh, saveAndStart, stop, restart, validateExternal, configureAndImportXiaoyanApi, clearApiImportResult: () => setApiImportResult(null), chooseFile, chooseDirectory };
}
