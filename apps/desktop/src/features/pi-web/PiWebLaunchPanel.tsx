      ...
      <div className="mx-auto max-w-4xl">
        ...
        <Card padding="lg">
          ...
          {draft.mode === "path" && !runtime.snapshot?.pathAvailable ? (
            <div className="mt-4 flex gap-2.5 rounded-2xl border border-amber-700/15 bg-amber-50/60 px-3.5 py-3 text-amber-900">
              <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0" />
              <p className="text-xs leading-5">未找到 Pi Web。请安装 Node.js 22.19+，再执行 <code className="font-mono">npm install -g @agegr/pi-web</code>。</p>
            </div>
          ) : null}
          ...
          <div className="mt-6 flex justify-end"><Button onClick={() => void runtime.saveAndStart(draft)} disabled={!canStart || runtime.busy}><Play className="h-3.5 w-3.5" />启动 Pi Web</Button></div>
        </Card>
      </div>
    </div>
  );
